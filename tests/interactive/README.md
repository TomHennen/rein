...
| 17 | **Static multi-repo happy launch** — the happy counterpart to row 11's refusal: a session hand-scoped to TWO same-owner, App-installed throwaway repos LAUNCHES cleanly (the #68 coverage gate clears for BOTH) and the agent can READ and DECLARE against each. Two `rein run --direct` legs sharing one 2-repo session; each declare fires a PLAIN in-scope confirm (session.Contains → NOT the AddRepo SCOPE-EXPANSION prompt that an out-of-scope repo would trip — row 6) | **COVERED** | `journey_multi_repo.py` → **`golden/multi_repo.txt`** |
