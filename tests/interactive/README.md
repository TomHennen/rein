...
- `reinharness.py` — binary build/locate, env loading, the `ReinRun` pexpect
  ...
  the real-agent frame API (`AGENT_TAG`/`fold_agent_frames`/`drop_agent_frames`:
  rendered `capture-pane` milestone snapshots that are SHOWN in the golden and DROPPED
  by the comparator) and `dismiss_claude_trust_dialog` (plumbing),
