...
Invariants (a break is exit 1, independent of the golden):
  - with NO preset configured, a preset host is REACHABLE (pypi.org -> 200),
    ...
    try:
        ...
        steps = [
            H.JourneyStep(
                ...
                extra_env={"REIN_SESSION_FILE": _pinned_session(repo, None), "REIN_SANDBOX_WORKDIR": workdir},
                timeout=180,
            ),
            H.JourneyStep(
        ...
        print("  default: pypi.org 200, example.com 000 — the dev preset opens registries only", flush=True)
