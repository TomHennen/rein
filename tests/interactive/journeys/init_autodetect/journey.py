"""init_autodetect — `rein init` repo-prompt default from the cwd git `origin` (#69/#78).

See README.md for the full description; journey-authoring rules are in
tests/interactive/CLAUDE.md.

Code note: each init run passes --no-alias --no-symlink --skip-mint-check, and REIN_APP_*
stays present so init keeps to the env path and NEVER reaches the ~25-minute
manifest/browser flow. The "checkout" is a bare `git init` + `remote add origin` at the
throwaway — enough for origin-URL detection, touching no real repo.
"""
