"""credential_boundary — the credential hide, proven by an INDEPENDENT scanner (bagel) (#59/#55).

See README.md for the full description; journey-authoring rules are in
tests/interactive/CLAUDE.md.

Code notes:
  * The fixture MUST live under the REAL $HOME (~/rein-bagel-fixture): deny-$HOME is
    what hides it, and (verified) a HOME relocated under /tmp is NOT hidden — an
    isolated-HOME harness would defeat the property under test.
  * The scan is scoped with --base-dirs and findings are reduced to the fixture path,
    so bagel's ambient probes (gh-auth, env — which ignore --base-dirs) add no
    machine-dependent noise: CONTROL=4, SANDBOXED=0.
"""
