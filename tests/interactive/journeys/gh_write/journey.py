"""gh_write — the in-sandbox `gh` / REST + GraphQL write boundary (#91, #101 gap 1).

See README.md for the full description; journey-authoring rules are in
tests/interactive/CLAUDE.md.

Code note: a REAL `pull_requests: write` or GraphQL-gate 403 on the `gh pr create` leg
(PHASE5 below) is a PRODUCT bug, not a test bug — exit 2, not drift.
"""
...
def main() -> int:
    ...
    if supplied:
        ...
        issue = H.create_issue(
            ...
            "Opened by journeys/gh_write/journey.py to prove that a SANDBOXED "
