"""git_author — the delegated commit author "<name> (via rein)" (CP4/B8).

See README.md for the full description; journey-authoring rules are in
tests/interactive/CLAUDE.md.

Code note: the `@AUTHOR …` sentinel line is ordinary tagged agent output, so the
delegated identity lands verbatim in the golden; the host-side API assertion is an
outcome (exit 2 on failure), not baked into the golden.
"""
...
def main() -> int:
    ...
    if supplied:
        ...
        issue = H.create_issue(
            ...
            "Opened by journeys/git_author/journey.py to demonstrate the "
