"""tmux_popup_approval — the DEFAULT approval surface inside $TMUX (#37).

See README.md for the full description; journey-authoring rules are in
tests/interactive/CLAUDE.md.

Code note: the popup's Form A is read off a RENDERED screen (pyte; #100), not from raw
ANSI that interleaves the live pane underneath. It is deterministic without any timer —
rein writes Form A once and BLOCKS on input, so the trailing `>` prompt line appearing on
screen IS the frame-complete signal (popup_forma_complete); only issue #/title/repo vary
...
def ceremony_script(repo: str, issue: int, good: str, bad: str) -> str:
    ...
    `--progress` on purpose (see journeys/write_ceremony/journey.py for the full why: git's
...
def main() -> int:
    ...
    if supplied:
        ...
        issue = H.create_issue(
            repo, title,
            "Opened by journeys/tmux_popup_approval/journey.py to "
