...
| Short-lived by default | No | No (up to unlimited) | Yes (8h) | Yes (~1h scoped tokens; per-run write approval) |
...
**Confirmation is approve-once-per-run, not per-write** (correction C4, refined to run-scoping). An early reading of the plan treated the write confirmation as per-push, which is UX-hostile for a productive session. The right ergonomic — and the one §2.2 always described — is a single confirmation per agent run that then covers every write for that run's lifetime. The approval is scoped to one `rein run` invocation via a per-run nonce (`REIN_RUN_ID`) and stored in per-run files, so concurrent runs are isolated, an approval can't leak across runs, and it is cleared when the run exits (no lingering global window). The run id's uniqueness is the bound — there is no security TTL. Implemented in `internal/approvals` (per-run records) + `rein approval grant/status/clear`. (Phase 0's first cut used a single global record keyed on session config with a 4h TTL; that was replaced by run-scoping because the global record let one run reuse another's approval and lingered after exit.)
...
**Anthropic's `sandbox-runtime` (srt)** (`anthropic-experimental/sandbox-runtime`, Apache 2.0, research preview). The primary sandbox integration target. Uses `sandbox-exec` on macOS and `bubblewrap` on Linux with HTTP and SOCKS5 proxies for network mediation. Validation §12.2 confirmed `srt` exposes a usable library API (`SandboxManager`, `FilterRequestCallback`, `network.mitmProxy.socketPath`, `parentProxy`) — no fork or upstream PR required to integrate. **Sharpened by the Phase 1 spike (2026-06-08, `docs/phase1-srt-spike-findings.md`):** of those, only **`network.mitmProxy.socketPath`** can inject a credential — `FilterRequestCallback` is allow/deny-only and `parentProxy` sees only an opaque CONNECT tunnel. rein registers an external Go MITM at that socket, TLS-terminates with its own CA, and injects host-aware auth (Bearer for api.github.com, Basic `x-access-token:` for github.com git transport). The spike also found the sandbox must `denyRead` ambient credential stores (e.g. `~/.config/gh/hosts.yml`) — network injection alone does not stop an agent reading the user's existing logins.
...
**Private-key custody — the keystore abstraction** (Phase 0.5). Each App's private key is written to disk (`~/.config/rein/<role>.pem`) at mode `0600`. This makes the earlier "no private key on disk" framing (§5.3 TM-G1, now corrected) false: the App PEM *is* on disk. To contain the resulting exposure, **all private-key reads go through `internal/keystore.Keystore`** (CLAUDE.md hard constraint #6). Two backends ship in Phase 0.5:
...
- **GitHub's installation-token mint TTL is fixed at ~1h** (correction C6). `POST /app/installations/{id}/access_tokens` does not accept a custom expiration, so the design's "5-minute write TTL" is not achievable at mint time. An attempt to tighten the effective window by revoking on the credential helper's `store` action backfired: git calls `store` MID-push (after `GET /info/refs`, before `POST /git-receive-pack`), so revoking there killed the token before the upload and every HTTPS push 401'd. The helper therefore revokes only on `erase` (git rejected the credential — the operation is over). The remaining gap — a *successful* push never triggers `erase` — is closed by **revoking on `rein run` child-exit** (issue #20): the helper appends each minted write token to a per-run ledger (`writes/<run-id>.jsonl`), and `rein run` best-effort revokes any still-valid entries when the wrapped agent exits, then clears the ledger. This is the reliable operation-complete signal git's `store`/`erase` could not safely provide. `rein-gh` still revokes on its own process exit. The exit-revoke requires persisting the write-token *value* to disk (mode 0600, deleted on run exit): unavoidable because GitHub's revoke API (`DELETE /installation/token`) is authenticated *by the token itself* — there is no revoke-by-id, so the revoking process must hold the value. The exposure is bounded by the run lifetime and adds nothing a same-uid process couldn't already reach during the run (the Shape B limitation in issue #7).

The Shape B effective write window for a git push is therefore the run lifetime (revoked on `rein run` child-exit per #20), falling back to the token's native ~1h TTL only when `rein run` is SIGINT/SIGKILLed before the exit-revoke fires; for `rein-gh` it is bounded by exit-revoke. The single-use + HEAD-pinning TOCTOU mitigation (TM-G6) is a Shape A property that depends on the proxy inspecting commits; Shape B does not inspect commits.
...
**TM-G1 GitHub App private key custody.** The original design assumed a Device Flow path with no App private key on disk. Phase 0.5's manifest flow changed this: each App's private key **is** written to disk at `~/.config/rein/<role>.pem`, mode `0600` (§4.2.4). The mitigation is the `internal/keystore` abstraction — all reads go through `Keystore.Get`, which enforces uid + mode `0o077` ownership checks (and O_NOFOLLOW against symlink swaps) on every read, failing closed on a loose-perm or wrong-owner PEM. This is a meaningful exposure to be honest about: a same-UID attacker who can defeat those checks (or who is the same uid) can read the App key, which is broad. Phase 1's daemon-backed and biometric keystore backends raise this bar; the file backend is the floor. Distinct from the broker's own signing key (§4.2.1.1), which has its own custody story.
...
| **C4: Per-write prompting is UX-hostile; §2.2 was always right** | §2.2 (session start ceremony) | A literal "single prompt presented to human for the push" was misread as per-write. Approve-once-per-run — scoped to a `rein run` invocation via a per-run nonce, cleared on exit, concurrent runs isolated — is the right pattern and aligns with §2.2. (Superseded the earlier single global 4h-TTL approval record.) |
...
| **C6: GitHub's installation-token mint TTL is fixed at ~1h** | §4.2.5 ("Shape B") | `POST /app/installations/{id}/access_tokens` accepts no custom expiration. Revoking on the helper's `store` action broke `git push` (store fires mid-operation), so the helper revokes only on `erase`. The successful-push gap is closed by revoking on `rein run` child-exit (#20): the helper ledgers each minted write token to `writes/<run-id>.jsonl` and `rein run` revokes still-valid entries on exit, bounding the effective window to the run lifetime (native ~1h TTL only if `rein run` is SIGINT/SIGKILLed first). Persisting the token value to disk (0600, deleted on exit) is unavoidable: revoke is authenticated by the token itself, with no revoke-by-id. `rein-gh` revokes on exit. |
