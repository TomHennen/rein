...
- **GitHub's installation-token mint TTL is fixed at ~1h** (correction C6). `POST /app/installations/{id}/access_tokens` does not accept a custom expiration, so the design's "5-minute write TTL" is not achievable at mint time. An attempt to tighten the effective window by revoking on the credential helper's `store` action backfired: git calls `store` MID-push (after `GET /info/refs`, before `POST /git-receive-pack`), so revoking there killed the token before the upload and every HTTPS push 401'd. The helper therefore revokes only on `erase` (git rejected the credential — the operation is over); a successful push's write token is left to expire on its native ~1h TTL. `rein-gh` still revokes on its own process exit. Tightening the git-push write window back toward "operation duration" needs a real operation-complete signal — `rein run` child-exit — tracked as a followup (issue #20).

The Shape B effective write window for a git push is therefore the token's native ~1h TTL (not promptly revoked, pending the issue #20 exit-revoke); for `rein-gh` it is bounded by exit-revoke. The single-use + HEAD-pinning TOCTOU mitigation (TM-G6) is a Shape A property that depends on the proxy inspecting commits; Shape B does not inspect commits.
...
| **C6: GitHub's installation-token mint TTL is fixed at ~1h** | §4.2.5 ("Shape B") | `POST /app/installations/{id}/access_tokens` accepts no custom expiration. Revoking on the helper's `store` action broke `git push` (store fires mid-operation), so the helper revokes only on `erase`; a pushed write token lives to its native ~1h TTL. Tighter bounding via `rein run` exit-revoke is a followup (#20). `rein-gh` revokes on exit. |
