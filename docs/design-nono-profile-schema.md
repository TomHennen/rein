...
**Status:** DONE. This gates the profile generator (`internal/nono/profile.go`). It
replaces the provisional struct in `design-nono-pivot.md` §2.2 — read the **§2.2
corrections** section below before coding `Build`.

**Scope:** Linux, nono **0.68.0** (`~/.local/bin/nono`), source commit `23d93fc`.
Everything here is checked against the real binary and/or the real source, not guessed
from the spike.

## How each fact was checked

Three levels of confidence are used throughout, and the distinction matters:

- **schema-verified** — shows up in `nono profile schema` output and/or is accepted or
...
- **launch-verified** — a profile using the field actually ran via
  ...
  field *shape is accepted, the profile validates, and nono applies the sandbox*. It does
  **not** exercise the dial to rein's upstream proxy, CA trust under real TLS, or AF_UNIX
  mediation under load — those stay P1 gates, not claims of this doc.

## Four things §2.2 got wrong about the shape

1. **The profile is NESTED, not flat.** §2.2 put `allow_domain`, `upstream_proxy`,
   `upstream_bypass`, `deny_credentials`, and `env` at the top level. The real top-level
   keys are `network`, `filesystem`, `linux`, `environment`, `groups`, `meta`, … The
   rein-relevant fields live under `network.*`, `filesystem.*`, `linux.*`,
   `environment.*`, and `groups.include`.
2. **There is no `external_proxy{url,auth}` object, and no way to give rein's listener a
   per-session password.** `network.upstream_proxy` (alias `network.external_proxy`) is a
   **bare `host:port` string**. Supplying an object with an `auth` field is **rejected at
   validate** (*"invalid type: map, expected a string"*). The internal
   `ExternalProxyAuth{keyring_account}` struct that exists in the source is (a) never
   filled from the profile — the CLI hardcodes `auth: None` — and (b) explicitly "not yet
   implemented," erroring if set. So upstream-proxy auth simply **does not work in
   0.68.0.** See the dedicated section below.
3. **`deny_credentials` is a policy GROUP, not a path list.** You turn it on with
   `groups.include: ["deny_credentials"]` (a fixed, `required` group that blocks known
   credential locations). You can't add arbitrary paths to it. §2.2's
   `DenyCredentials []string` and "MUST include the profile path itself" are wrong on the
   mechanism.
4. **Arbitrary env injection works — via `environment.set_vars`, not a top-level `env`** —
...
| upstream proxy → rein | `network.upstream_proxy` | **string `host:port`** | schema + source | Alias: `network.external_proxy` (legacy). **Bare host:port — NOT a URL.** Source dials it with `TcpStream::connect(addr)`, so `"http://127.0.0.1:PORT"` (as §2.2 wrote) is wrong — use `"127.0.0.1:PORT"`. (Not dial-tested here; `echo ok` doesn't reach upstream.) |
| CDN → direct | `network.upstream_bypass` | array of string (exact host or `*.` wildcard) | schema + launch | Alias: `external_proxy_bypass`. Hosts here skip the upstream proxy and go **direct** (never reach rein). Requires `upstream_proxy` to be set, or validate errors. |
...
The task was to settle definitively whether the per-session proxy secret lives in the
profile JSON or in an OS keyring. **Answer: neither — nono 0.68.0 can't carry
...
**What this means for the design (§3a/§7).** §3a's earlier plan — rein's listener requires
a per-session secret that nono sends on every CONNECT — **cannot be built on 0.68.0.** But
this is not a hole, because the loopback-capability regression it was meant to close is
**closed a different way, and it's tested:** a nono-sandboxed agent's raw `connect()` to an
arbitrary loopback port is refused; nono lets the agent reach only nono's own proxy on
loopback, so the agent can't reach rein's listener directly (design §3a). rein's listener
therefore needs no password. §3a itself always treated proxy-auth as defense-in-depth, not
the security boundary — the primary gates stand: declare + tier classifier + **downstream**
token injection (the token value never enters the sandbox) + approval-channel isolation via
`af_unix_mediation`. The secret-hiding sub-concern (the profile JSON must be agent-unreadable)
is simply gone — there is no secret. The spike used a no-auth loopback relay, which is now
the intended production shape.

## The two proxy hops — do not conflate (both observed at launch)

Launching the composed profile with `-- /usr/bin/env` shows nono manages the **agent→nono**
hop itself:

- **agent → nono** (hop 1): nono **injects and overrides** `HTTP_PROXY`, `HTTPS_PROXY` (and
  lowercase) to point at **nono's own** loopback proxy
  ...
  credential is why git needs **`http.proxyAuthMethod=basic`** (spike finding, preserved).
  `NONO_PROXY_TOKEN` **is agent-visible and that is fine** — it authenticates only to nono's
  own proxy; the GitHub token is injected downstream by rein and never enters the sandbox.
- **nono → rein** (hop 2): the `network.upstream_proxy` field. **Bare host:port, no auth**
  (previous section). This is the opaque CONNECT tunnel rein TLS-terminates.

**Correction to design §3c.** §3c said rein's profile `env` must set `HTTPS_PROXY`/
`HTTP_PROXY` (→ nono's proxy) and `NO_PROXY` (CDN bypass). **nono owns those vars and
overrides any `set_vars` you write for them** — proved: a `set_vars`
`HTTPS_PROXY=http://127.0.0.1:47821` was replaced by nono's own value at launch. So: rein
must NOT set the proxy env vars (nono does), and CDN bypass is done with
`network.upstream_bypass` (nono → direct), **not** the agent's `NO_PROXY` (which nono pins
to `localhost,127.0.0.1`). Non-git tools still get proxied because nono sets their
`HTTPS_PROXY` for them.
...
(`nono run --profile … -- echo ok` → `ok`, exit 0). It composes every rein knob that 0.68.0
actually supports: github + api + declare host allowed, upstream proxy to a loopback rein
listener, CDN in `upstream_bypass`, the `deny_credentials` group, `af_unix_mediation:
pathname`, CA-trust env (all four vars), and `http.proxyAuthMethod=basic` + `http.postBuffer`
via `GIT_CONFIG_*` env.
...
The generator should emit nested structs mirroring nono's real schema. `ExternalProxy` and
`ProxyAuth` are **deleted** (no such object; auth can't be wired). `Env` and
`DenyCredentials` move; `af_unix_mediation`, the unix-socket allowlist, and the CA read
grant are **added**.
...
`Build`'s invariants from §2.2 mostly carry, with corrections:
...
- `ExtraDomains` never injected — unchanged (enforced in the proxy, not the profile).
...
- Drop the `DenyReadPaths` / "profile path in deny_credentials" logic: `deny_credentials`
  is a fixed group, `filesystem.deny` is a **Linux no-op** (Landlock has no deny-within-allow),
  and there's no profile secret to hide anyway (auth unimplemented). The filesystem is
  default-deny, so the rule is **"grant nothing you don't need,"** not "deny the secret."

## Gotchas for whoever codes the generator (read these)

1. **Schema drift.** `nono profile schema` **omits** the `filesystem.unix_socket*` fields
   (and possibly others) that the profile actually accepts. Do NOT treat the schema dump as
   complete — cross-check `nono profile guide` + source. The `unix_socket*` grant fields are
   the load-bearing example (they gate the §3e approval-channel isolation and don't appear
...
6. **No proxy-auth.** Anything requiring nono to authenticate to rein's listener
   (`external_proxy.auth`, a per-session secret) can't be built on 0.68.0 — nono's loopback
   mediation is what protects rein's listener instead (§3a).
