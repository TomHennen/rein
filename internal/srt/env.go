...
// EnvAgentWorktrees, EnvAgentCloneDir, EnvUpstreamIntentFile,
// EnvDisableClaudeMCP, DisableClaudeMCPFromEnv, and the REIN_IN_SANDBOX_*
// consts moved to internal/agentenv — the agent-visible env contract is not
// srt-specific, so a future nono launcher sets the same vars without
// importing srt (docs/design-nono-pivot.md §5/§7). Re-exported here so
// existing srt callers are unaffected; see agentenv for the doc comments.
const (
	EnvAgentWorktrees     = agentenv.EnvAgentWorktrees
	EnvAgentCloneDir      = agentenv.EnvAgentCloneDir
	EnvUpstreamIntentFile = agentenv.EnvUpstreamIntentFile
	EnvDisableClaudeMCP   = agentenv.EnvDisableClaudeMCP
	EnvInSandbox          = agentenv.EnvInSandbox
	EnvInSandboxWorktree  = agentenv.EnvInSandboxWorktree
	EnvInSandboxHome      = agentenv.EnvInSandboxHome
)

var DisableClaudeMCPFromEnv = agentenv.DisableClaudeMCPFromEnv
...
// caEnvVars moved to internal/sandboxutil.CAEnvVars (substrate-neutral: a
// future nono profile generator needs the same four vars without importing
// srt — docs/design-nono-pivot.md §3b/§5/§7). On the mitmProxy path srt sets
// NO CA vars (mitmCA is undefined), so rein must point every client's trust
// store at the bundle itself.
var caEnvVars = sandboxutil.CAEnvVars
